...
def get_yahoo_data(ticker):import os
import requests
...
# 3. Apply the corrected 15.5% Indian Market Premium 
# (Accounts for basic customs duties, AIDC, bank TT selling rates, 3% GST, and physical delivery)
india_multiplier = 1.155 
odisha_gold_24k = gold_10g_base * india_multiplier
odisha_silver_1kg = silver_1kg_base * india_multiplier

# Calculate 22K Gold (Jewelry purity) from the 24K base rate
odisha_gold_22k = odisha_gold_24k * (22 / 24)

# 4. Generate Market Mood Analysis based on the day's changes
if gold_change > 0.5:
...
# 5. Format the Telegram markdown bulletin message
message = f"""🌅 *Bhubaneswar Metal Bulletin* 🌅
...
_(Calculated using live global feeds adjusted for national duties, cess, and 3% local GST. Retail making charges are not included.)_"""
